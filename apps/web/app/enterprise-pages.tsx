...
import { praDocsUrl } from "@/lib/content";
import { Arrow, Breadcrumbs, CodeBlock, CtaBand, PageHero, SectionTitle, Status } from "./components";
    ...
    <>
      ...
      <section className="section section-tint" id="evaluation">
        <div className="wrap">
          ...
          <div className="assessment-handoff"><div><span className="kicker">One artifact contract</span><h3>Commercial work builds on the open assessment workflow.</h3><p>The public CLI creates the workload configuration, environment receipt, quality evidence, metrics, recommendation, and report. A Comprehensive Evaluation adds jointly defined gates, production-like traffic, specialist analysis, customer-specific calibration, capacity economics, an acceptance suite, and a scoped recommendation.</p><a className="inline-link" href={praDocsUrl("cli.html")}>Review the assessment commands <Arrow /></a></div><CodeBlock>{`pra assess init customer-workload
# Edit the workload and acceptance gates
pra assess run .pra/assessments/customer-workload \
  --measurements matched-results.json
pra assess report .pra/assessments/customer-workload \
  --format html`}</CodeBlock></div>
...
export function GatewayReworkedPage() {
  return <><Breadcrumbs items={[{ label: "PRA", href: "/products/pra" }, { label: "Gateway" }]} /><PageHero eyebrow="PRA Gateway · Available compatibility boundary" title="Put Selected Context in front of an existing model endpoint." intro="The open-source Gateway preserves an OpenAI-compatible application boundary, coordinates typed resources and sessions, negotiates backend capabilities, selects authorized ordinary context, and makes fallback explicit." actions={<><Link className="button" href="/developers/quickstart">Start the Gateway <Arrow /></Link><a className="button button-ghost" href={praDocsUrl("deployment/gateway.html")}>Gateway operations</a></>} /><section className="engine-summary"><div className="wrap"><Status>Available</Status><span>Selected Context, typed transport, sessions, deltas, streaming, and traces</span></div></section><section className="section wrap"><div className="compatibility-flow" aria-label="PRA Gateway deployment"><div><span>Existing application</span><small>messages · tools · session</small></div><i>→</i><div className="accent"><span>PRA Gateway</span><small>identity · authorization · planning · selection</small></div><i>→</i><div><span>Existing model server</span><small>effective negotiated transport</small></div></div><div className="gateway-capabilities"><article><span>01</span><h3>Normalize and protect</h3><p>Preserve tenant, session, task, resource identity, authorization scope, and request budgets.</p></article><article><span>02</span><h3>Plan realization</h3><p>Distinguish context already visible, natively available, newly materialized, or invalid.</p></article><article><span>03</span><h3>Negotiate explicitly</h3><p>Keep required capabilities intact, use an allowed fallback, or return a clear error.</p></article><article><span>04</span><h3>Commit and trace</h3><p>Report effective transport, selected resources, downgrades, bytes, deltas, reuse, and native use.</p></article></div></section><section className="section section-tint"><div className="wrap"><SectionTitle eyebrow="Operating modes" title="Ordinary engines and PRA-aware endpoints share one boundary." copy="Fallback is never silent. A request requiring an unsupported capability fails unless text fallback is explicitly allowed." /><div className="gateway-mode-grid"><article><Status>Ordinary engine</Status><h3>Selected Context</h3><p>The Gateway renders authorized selected resources as labeled visible context before calling the downstream engine.</p><CodeBlock>{`pra gateway serve \\\n  --mode selected-context \\\n  --backend vllm \\\n  --backend-url http://127.0.0.1:8000/v1`}</CodeBlock></article><article><Status>Capable endpoint</Status><h3>Typed transport</h3><p>The Gateway preserves typed records, resource inventories or deltas, task/session metadata, and policy for a backend that advertises those capabilities.</p><CodeBlock>{`pra gateway serve \\\n  --mode typed-transport \\\n  --backend sglang \\\n  --backend-url http://127.0.0.1:30000`}</CodeBlock></article></div></div></section><section className="section wrap"><SectionTitle eyebrow="Sessions and operations" title="Logical durability without pretending opaque engine state survived." copy="Durable snapshots reconstruct logical visibility after a Gateway restart. Engine sessions and cache handles are re-established only when capability and identity checks support reuse." /><div className="feature-cards"><article><span>01</span><h3>Tenant + session + model keys</h3></article><article><span>02</span><h3>Add, update, and remove deltas</h3></article><article><span>03</span><h3>Prefix and semantic reuse stay distinct</h3></article><article><span>04</span><h3>Health, capability, and session endpoints</h3></article></div><div className="technical-link-grid"><a href={praDocsUrl("protocol.html")}><strong>Wire protocol</strong><span>Typed request and capability contract</span><Arrow /></a><a href={praDocsUrl("observability.html")}><strong>Observability</strong><span>OTel traces, Prometheus metrics, and privacy defaults</span><Arrow /></a><a href={praDocsUrl("deployment/engines.html")}><strong>Engine deployment</strong><span>Choose by supported semantics and evidence</span><Arrow /></a></div></section><section className="section section-tint"><div className="wrap"><SectionTitle eyebrow="Security boundary" title="Logical resources cross the application boundary. Native memory does not." /><div className="risk-grid"><article><h3>No raw K/V on the wire</h3><p>Only an engine adapter creates and consumes model-native memory inside the engine process.</p></article><article><h3>Authorization independent of relevance</h3><p>A relevant or resident resource is still excluded when the caller cannot materialize it.</p></article><article><h3>Cross-tenant rejection</h3><p>Resource ownership is checked against the request tenant and remains attached during transport.</p></article><article><h3>Credentials stay out of metadata</h3><p>Credentials belong in transport headers or provider configuration, not typed PRA records.</p></article><article><h3>Explicit invalidation</h3><p>Capability loss, stale versions, and incompatible prefix changes force safe resynchronization.</p></article><article><h3>Ordinary-text compatibility floor</h3><p>Selected Context remains available without an engine patch or model retraining.</p></article></div></div></section><CtaBand title="Evaluate the Gateway against your current endpoint." /></>;
}
